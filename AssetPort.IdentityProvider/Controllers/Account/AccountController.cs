...
namespace AssetPort.IdentityProvider.Controllers.Account
{
    ...
    public class AccountController : Controller
    {
        ...
        public async Task<IActionResult> Login(LoginInputModel model, string button)
        {
            ...
            if (ModelState.IsValid)
            {
                ...
                if (_userRepository.AreUserCredentialsValid(model.Username, model.Password))
                {
                    ...
                    // Send code to user (i.e. Text to Phone, E-mail)
                    // ??
                    ...
                    // For 2FA, comment this
                    // Issue authentication cookie with subject ID and username
                    //await HttpContext.SignInAsync(user.SubjectId, user.Username, props);

                    // For 2FA, comment this too
        ...
        public async Task<IActionResult> ExternalLoginCallback()
        {
            ...
            if (user == null)
            {
                // User wasn't found by Provider, but maybe one exists with the same e-mail address
                if (provider == "Facebook")
                {
                    var email = claims.FirstOrDefault(c => c.Type == "http://schemas.xmlsoap.org/ws/2005/05/identity/claims/emailaddress" || c.Type == "http://schemas.xmlsoap.org/ws/2005/05/identity/claims/email");
                    if (email != null)
                    {
